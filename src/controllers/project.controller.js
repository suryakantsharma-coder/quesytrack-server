...
const createProject = asyncHandler(async (req, res) => {
  ...
  const { projectName, projectDescription, overdue, progress, status, startedAt } = body;
...
const searchProjects = asyncHandler(async (req, res) => {
  if (!requireCompany(req, res)) return;
  const paginationParams = parsePaginationParams(req.query);
  const filter = buildSearchFilter(req.query, ['projectName', 'projectDescription', 'projectId'], {
    dateField: 'startedAt',
  });
  addCompanyFilter(filter, req);
  const populateOptions = [{ path: 'createdBy', select: 'name email role' }];
...
const getCalibrationsByProjectId = asyncHandler(async (req, res) => {
  ...
  return successResponse(res, 200, 'Calibrations retrieved successfully', { calibrations, pagination });
});
