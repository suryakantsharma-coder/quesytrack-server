...
const createCalibration = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    const project = await Project.findOne({ projectId: projectId.trim(), company: req.user.company });
    if (!project) {
...
const getCalibrations = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    const project = await Project.findOne({ projectId: filter.projectId.trim(), company: req.user.company });
  ...
  const calibrationsWithGauge = await Promise.all(
    calibrations.map(async (calibration) => {
      ...
      if (calibrationObj.gaugeId) {
        ...
        calibrationObj.gauge = await Gauge.findOne(gaugeFilter).select('gaugeName gaugeId');
...
const searchCalibrations = asyncHandler(async (req, res) => {
  if (!requireCompany(req, res)) return;
  const paginationParams = parsePaginationParams(req.query);
  let filter = buildSearchFilter(req.query, ['calibrationId', 'calibratedBy', 'certificateNumber'], {
...
const updateCalibration = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    const project = await Project.findOne({ projectId: body.projectId.trim(), company: req.user.company });
    if (!project) {
      return errorResponse(res, 400, 'Project not found with the provided projectId');
    }
